...
def build(config_path=None, output=None):
    ...
    def card(o):
        ...
        period = f' <span class="period">/ {e(o.get("billing_period", "month"))}</span>' if o.get('price') is not None else ''
        return f'''<article class="card"><div class="card-top"><span class="provider-name">{e(p['name'])}</span><span class="tag">Official source</span></div><h3><a href="/deals/{e(o['slug'])}/">{e(o['title'])}</a></h3><p class="price">{price(o)}{period}</p><p class="summary">{e(o.get('category','Hosting'))}</p><dl>{''.join(f'<div><dt>{e(x.split(" ")[0])}</dt><dd>{e(x)}</dd></div>' for x in terms) or '<div><dt>Terms</dt><dd>See source</dd></div>'}</dl><a class="button" href="/deals/{e(o['slug'])}/">View terms</a><p class="capture">Captured {e(date_text(o['fetched_at']))}</p></article>'''
    ...
    rows=''.join(f'<tr><td><strong>{e(byid[o["provider"]]["name"])}</strong><span>{e(o["title"])}</span></td><td>{e(price(o))}{("<span>per "+e(o.get("billing_period","month"))+"</span>") if o.get("price") is not None else ""}</td><td>{e(str(o.get("commitment_months","Unknown")))}</td><td>{e(money(o["renewal_price"])+"/mo" if o.get("renewal_price") is not None else "Unknown")}</td><td><a href="{e(o["source_url"])}" rel="noopener noreferrer">Official page ↗</a></td></tr>' for o in offers)
    ...
    snapshot_time=payload.get('generated_at')
    pages=[('/',snapshot_time),('/providers/',snapshot_time),('/compare/',snapshot_time),('/methodology/',snapshot_time),('/disclosure/',snapshot_time),('/privacy/',snapshot_time)]
    pages += [(f'/providers/{p["id"]}/', max((o.get('fetched_at', snapshot_time) for o in offers if o['provider']==p['id']), default=snapshot_time)) for p in cfg['providers']]
    pages += [(f'/deals/{o["slug"]}/',o.get('fetched_at',snapshot_time)) for o in offers]
    write(Path('sitemap.xml'),'<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+''.join('<url><loc>'+e(domain+path)+'</loc><lastmod>'+e(lastmod)+'</lastmod></url>' for path,lastmod in pages)+'</urlset>')
